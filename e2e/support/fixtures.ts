...
//
// Every Playwright worker boots its own Rust API (own port, own throwaway
...
// backend (see vite.config.ts). Specs import { test, expect } from here
// instead of '@playwright/test'; e2e/support/api.ts picks the worker's port
// up from E2E_API_PORT, which is per-process because each worker is one.
import { spawn } from 'node:child_process';
import { test as base, expect } from '@playwright/test';

/** Worker-scoped fixtures: one Rust API per Playwright worker. */
export interface E2EWorkerFixtures {
  /** Port of this worker's private Rust API (5100 + workerIndex). */
  apiPort: number;
}

export const test = base.extend<Record<never, never>, E2EWorkerFixtures>({
  apiPort: [
    // Empty destructuring pattern is required: Playwright parses it to
    ...
    async ({}, run, workerInfo) => {
      ...
      try {
        if (proc.pid === undefined) throw new Error('spawn produced no pid');
