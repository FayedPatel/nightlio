...
import type { LoginResponse, Preferences } from '../src/types/api';
...
test('settings theme picker saves the choice to the account', async ({ page, request, apiPort }) => {
  ...
  const { token }: LoginResponse = await login.json();
  const prefs = await request.get(`http://localhost:${apiPort}/api/preferences`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  const prefsBody: Preferences = await prefs.json();
  expect(prefsBody.theme).toBe('synthwave');
