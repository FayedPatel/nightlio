...
fn make_app() -> TestApp {
    ...
    TestApp {
        ...
        // Bootstrap seeds the default self-host user with id 1.
...
/// options 1..=27).
fn seed_activities_group(db_path: &str) {
...
async fn unauthenticated_requests_match_fixtures() {
    ...
    let (app, token) = (harness.app.clone(), harness.token.clone());
    for name in [
        "mood_post_401",
        "moods_get_401",
        "mood_get_401",
