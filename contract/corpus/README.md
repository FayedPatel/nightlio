...
**These goldens are frozen.** They were generated by `build_corpus.py` against
the Python bootstrap, which no longer exists in the tree — the Flask API was
deleted in the Rust rewrite. Regenerating them would mean checking out a
pre-rewrite commit and running the script there; `build_corpus.py` is kept in
this directory for reference (it documents exactly how each snapshot was built
and how the dumps are normalized), not as a runnable step.

(For the record of how they were produced: the script removed and rebuilt the
generated `.db`/`.pragmas.txt`/`.counts.txt` files, leaving this README and
itself in place. Row timestamps it inserts are pinned to
`2026-08-15 00:00:00`; rows created by the bootstrap itself — the seeded
self-host user and the default groups/options — carry `CURRENT_TIMESTAMP`, so
the `.db` files were never byte-identical across regenerations. The pragma and
count dumps are.)
...
| `build_corpus.py` | The generator that produced the files above, retained for reference only. It imports the deleted Python bootstrap, so it is not runnable against the current tree. |
