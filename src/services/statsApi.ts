...
//
// Lives in its own module (rather than api.ts) so it can land without
// touching src/services/api.ts. It delegates every call to the shared
...
import type { Digest, ExtendedStats, Heatmap } from '../types/api';
...
const statsApi = {
  ...
  getExtendedStatistics(): Promise<ExtendedStats> {
    return apiService.request<ExtendedStats>('/api/statistics/extended');
  },
  ...
  getHeatmap(year?: number | null): Promise<Heatmap> {
    const params = new URLSearchParams();
    if (year != null) params.set('year', String(year));
    const q = params.toString();
    return apiService.request<Heatmap>(`/api/statistics/heatmap${q ? `?${q}` : ''}`);
  },
  ...
  getMonthlyDigest(year?: number | null, month?: number | null): Promise<Digest> {
    const params = new URLSearchParams();
    if (year != null) params.set('year', String(year));
    if (month != null) params.set('month', String(month));
    const q = params.toString();
    return apiService.request<Digest>(`/api/statistics/digest${q ? `?${q}` : ''}`);
  },
};
