...
// This file compiles under tsconfig.worker.json (lib WebWorker, no DOM), where
// `self` is a plain WorkerGlobalScope; redeclare it as the service worker scope
// carrying the __WB_MANIFEST placeholder vite-plugin-pwa replaces at build time.
...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(
    fetch(event.request).catch(async () => {
      // offline.html is always in the precache, so the match always resolves;
      // the Response.error() fallback only exists to satisfy respondWith's
      // `Response | undefined` typing and never fires at runtime.
