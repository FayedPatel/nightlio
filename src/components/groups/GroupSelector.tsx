  ...
  return (
    ...
    ? 'linear-gradient(135deg, var(--accent-bg), var(--accent-bg-2))'
          ...
          color: selectedOptions.includes(option.id) ? 'white' : 'var(--text)',
                ...
                onMouseEnter={(e) => {
                  // currentTarget is the typed equivalent of target here: the
                  // button's only child is a text node, so mouseenter always
                  // fires with the button itself as the target.
