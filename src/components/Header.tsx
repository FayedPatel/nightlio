...
const SEARCH_FIELDS: readonly (keyof MoodEntry)[] = ['content', 'date'];
...
const shouldSkipShortcut = (target: EventTarget | null): boolean => {
  ...
  // instanceof narrows to HTMLElement for isContentEditable; non-HTML
  // elements (e.g. SVG) were falsy here before, so behavior is unchanged.
...
interface HeaderProps {
  ...
  // Matches SearchBar's onSearch contract — SearchBar is typed against the
  // base MoodEntry, so the handler must accept that wider element type.
