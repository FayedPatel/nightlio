...
const Modal = ({ open, title, headerActions, children, onClose, maxWidth = 520 }: ModalProps) => {
  ...
  const dragRef = useRef<DragState>({ active: false, startY: 0, baseY: 0, pointerId: null });
  ...
  const modal = (
    <div className="ui-modal-overlay" aria-modal="true" role="dialog">
      {/* Backdrop click-to-close is a pointer convenience; the keyboard
          equivalent is the Escape handler above, so the backdrop itself is
          presentational (it conveys nothing to assistive tech). */}
