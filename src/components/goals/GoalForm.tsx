...
/** Imperative surface exposed via ref (right-rail quick suggestions prefill). */
export interface GoalFormHandle {
  prefill: (title: string, description: string) => void;
}

interface GoalFormProps {
  onSubmit: (goal: GoalFormData) => void | Promise<void>;
  onCancel: () => void;
...
const GoalForm = forwardRef<GoalFormHandle, GoalFormProps>(({ onSubmit, onCancel, showInlineSuggestions = true }, ref) => {
  ...
  // Focus management instead of the autoFocus attribute (jsx-a11y/no-autofocus):
  // this form only mounts after the user chooses "New Goal", so moving focus to
  // the first field is a deliberate post-navigation focus target, not a
  // page-load autofocus.
  useEffect(() => {
  ...
  return (
    <div style={{ maxWidth: '600px' }}>
      <button
        ...
        // MouseEvent.target is EventTarget in the DOM types; the original code
        // styles the hovered node itself, so a narrow cast preserves behavior.
      ...
      <label htmlFor="goal-desc" style={{ 
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div role="group" aria-labelledby="goal-frequency-label" style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <button
            ...
            // Same narrow EventTarget -> HTMLElement cast as the back button above.
            onMouseEnter={(e) => { (e.target as HTMLElement).style.background = 'var(--accent-bg-softer)'; }}
