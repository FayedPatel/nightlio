...
import type { WeeklyMoodPoint } from '../../utils/moodUtils';
import type { RollingAveragePoint, Statistics } from '../../types/api';
...
import type { RollingOverlayPoint } from './extendedStatsUtils';

export type TrendChartPoint = WeeklyMoodPoint & RollingOverlayPoint;
...
const useStatisticsViewData = (
  ...
  range: number,
  rollingSeries: readonly RollingAveragePoint[] | null = null,
) => {
  ...
  const trendChartData = useMemo<TrendChartPoint[]>(
    () => weeklyMoodData.map((point, index) => ({
      ...point,
      ...(rollingOverlay[index] ?? { avg7: null, avg30: null }),
    })),
  ...
  const bestDayCount = useMemo(() => {
    const counts = Object.values(moodDistribution ?? {}).filter(
