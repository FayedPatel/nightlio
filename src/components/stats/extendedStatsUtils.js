...
export const moodColorVar = (averageMood) => {
...
export const buildRollingOverlay = (series, days) => {
...
export const buildWeekdayChartData = (weekdayAverages) =>
...
export const buildVolatilityCard = (volatility, loading = false) => {
...
export const normalizeTagCorrelations = (rows) =>
  (rows ?? []).map((row) => ({
...
export const normalizeGoalCorrelations = (rows) =>
  (rows ?? []).map((row) => ({
    id: `goal-${row.goal_id}`,
    name: row.goal_name,
    detail: null,
...
export const splitCorrelationRows = (rows, minCount = MIN_CORRELATION_SAMPLE) => {
...
export const buildHeatmapGrid = (year, days) => {
...
export const formatSignedDelta = (value, digits = 2) => {
