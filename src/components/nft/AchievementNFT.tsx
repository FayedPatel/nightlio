...
interface AchievementNFTProps {
  /**
   * Accepts both full API rows (Achievement) and the locked-achievement
   * metadata literals in AchievementsView, which lack id/earned_at/nft_*.
   */
  achievement: Pick<Achievement, 'name' | 'description' | 'icon' | 'rarity'>;
  isUnlocked?: boolean;
...
const AchievementNFT = ({ achievement, isUnlocked = true, progressValue, progressMax }: AchievementNFTProps) => {
  ...
  const commonStyle: RarityStyle = { bg: 'color-mix(in oklab, var(--text) 12%, transparent)', text: 'var(--text)', border: 'color-mix(in oklab, var(--text), transparent 60%)' };
  ...
  const r = rarityStyles[rarityToken] ?? commonStyle;
